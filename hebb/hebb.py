...
class HebbianConvTransposed2d(HebbianConv2d):
	"""
	A 2d convolutional layer that learns through Hebbian plasticity
	"""
	...
	def update(self, x, y):
		...
		if self.mode == self.MODE_CONTRASTIVE:
			...
			if self.uniformity:
				...
				L *= torch.conv_transpose2d(uniformity_map.reshape(x.size(0), 1, x.size(1), x.size(2)), torch.ones([1, 1, *self.kernel_size], device=x.device, dtype=x.dtype), self.stride).reshape(-1, 1)
