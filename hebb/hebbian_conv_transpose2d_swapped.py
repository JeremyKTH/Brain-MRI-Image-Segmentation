...
class HebbianConvTranspose2dSwapped(HebbianConv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, w_nrm=True, act=nn.ReLU(inplace=True),
                 mode=HebbianUpdateMode.SWTA_T, k=0.02, patchwise=False, contrast=1., uniformity=False):
        """
    ...
    def update(self, x, y):
        ...
        if self.mode not in [HebbianUpdateMode.SoftWinnerTakesAll,
                             HebbianUpdateMode.HebbianPCA,
                             HebbianUpdateMode.SWTA_T,
                             HebbianUpdateMode.HPCA_T,
                             HebbianUpdateMode.CONTRASTIVE]:
            raise NotImplementedError(
                "Learning mode {} unavailable for {} layer".format(self.mode, self.__class__.__name__))

        if self.mode in [HebbianUpdateMode.SoftWinnerTakesAll, HebbianUpdateMode.HebbianPCA]:
            # In case of swta-type or hpca-type learning, use the learning rules for ordinary convolution,
            # but exchanging x and y
            super().update(y, x)

        if self.mode == HebbianUpdateMode.SWTA_T:
            ...
            r = r.permute(
                ...
                -1, self.out_channels, self.kernel_size[0] * self.kernel_size[1]
            ).permute(2,  1, 0)

            dec = r.sum(2, keepdim=True) * self.weight.permute(2, 3, 1, 0).reshape(-1, self.out_channels,
                                                                                   self.in_channels)
            if self.patchwise:
                dec = dec.sum(dim=0, keepdim=True)

            self.delta_w[:, :, :, :] = (
                    r.matmul(x.permute(0, 2, 3, 1).reshape(1, -1, x.size(1))) - dec
            ).permute(2, 1, 0).reshape_as(
                self.weight
            )

        if self.mode == HebbianUpdateMode.HPCA_T:
            ...
            mask = (torch.arange(
                ...
                device=x.device,
                dtype=x.dtype
            ).unsqueeze(0).repeat(self.out_channels, 1).le(
                torch.arange(
                    self.out_channels,
                    device=x.device,
                    dtype=x.dtype)
            ...
            dec = (r.matmul(r.transpose(-2, -1)) * mask.unsqueeze(0)).matmul(
